<?php
/**
 * vida functions and definitions
 *
...
function vida_setup() {
	/*
		...
		* If you're building a theme based on vida, use a find and replace
		* to change 'vida' to the name of your theme in all the template files.
		*/
	load_theme_textdomain( 'vida', get_template_directory() . '/languages' );
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'vida' ),
		)
	);
...
add_action( 'after_setup_theme', 'vida_setup' );
 ...
 */
function vida_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'vida_content_width', 640 );
}
add_action( 'after_setup_theme', 'vida_content_width', 0 );
...
function vida_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'vida' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'vida' ),
...
add_action( 'widgets_init', 'vida_widgets_init' );
